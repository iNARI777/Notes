...
在Class中、字段表中以及方法表中都存在属性表(Attribute_info)。在《Java虚拟机规范（Java SE 7）》中，虚拟机可以识别的属性有21项，虚拟机运行的时候会自动忽略不能识别的属性表。

对于每个属性，其名称都需要从常量池中引用一个CONSTANT_Utf8_info类型的常量来表示，而属性的结构则是完全自定义的。属性表的结构如下图所示：

![](image/class/7.png)

具体属性的作用可见书上P182-P196

## 2 字节码指令简介

Java虚拟机指令是由一个1字节的操作码和0个至多个操作数组成，但是由于Java虚拟机采用面向操作数栈而不是寄存器的架构，所以字节码指令一般没有操作数，只有一个操作码。

#### 2.1 字节码与数据类型

在Java虚拟机指令中，大多数操作码都包含了其操作所对应的数据的类型，但是由于数据类型与操作的组合可能会多与一个字节所能表达的256种操作，所以很多的数据类型是没有其相应的操作的，比如byte、short、char甚至boolean，他们会在编译时或运行时被扩展为int型，并使用int型的操作对这些数据类型进行操作。（数据类型支持表在P198，描述了各种操作所对应的数据类型）

#### 2.2 加载和存储指令

加载和存储指令用于将数据在栈帧中的局部变量在局部变量表和操作数栈之间来回传输。

* 将局部变量加载到操作数栈：iload, iload_<n>, lload, lload_<n>, fload, fload_<n>, dload, dload_<n>, aload, aload_<n>
* 将局部变量从操作数栈存储到局部变量表：将上述load变为store
* 将一个常量加载到操作数栈：bipush, sipush, ldc, ldc_w, ldc2_w, aconst_null, iconst_ml, iconst_<n>, iconst_<l>, iconst_<f>, iconst_<d>
